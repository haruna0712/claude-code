...
| E2Eテストシナリオ / stg実行メモ                           | [docs/specs/repost-quote-e2e-scenarios.md](./docs/specs/repost-quote-e2e-scenarios.md), [docs/local/e2e-stg.md](./docs/local/e2e-stg.md), `client/e2e/` |
...
6. 実装が終わったら **必ず実機の画面で操作して** 仕様書通りの振る舞いになっているか **Claude 自身が検証**する。手順:
   ...
   - **必ず実行して全 step が pass することを確認**する:
     - **第一選択 — Playwright (stg)**: `client/e2e/<feature>.spec.ts` を `PLAYWRIGHT_BASE_URL=https://stg.codeplace.me` + USER1/USER2 env で run。stg は main merge → CD で自動デプロイされる。env (パスワード等) が手元に無いときは **ハルナさんに尋ねる**。spec doc を書いただけ / コードを書いただけで「stg で実行する想定」と切り上げてはいけない (`#499` での反省)。
     - **第二選択 — browser MCP / Chrome 手動**: 認証情報や stg 反映待ちで Playwright を即時 run できないとき、**Playwright MCP (`mcp__playwright__*`) や Chrome の手動操作** で代替検証する。spec シナリオ通りに 画面操作 → 期待結果を確認し、結果を会話で報告する (スクリーンショット推奨)。
...
- **「実機検証はユーザー側で」と勝手に切り上げない**: spec doc にシナリオを書き、E2E spec ファイルを書いただけでは **未検証**。stg Playwright か Playwright MCP / Chrome 手動で **Claude 自身が画面を踏む** までが完了条件 (§4.5 step 6)。env (パスワード等) が手元に無ければハルナさんに尋ねる。`#499` で踏み抜いた反省。
