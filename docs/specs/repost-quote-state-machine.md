...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `REPOST`             | **行ごと TL から消える**。「@user がリポストしました」バナー + 元 tweet 本体の組で 1 行になっているので、本体が消えると行ごと消滅。  | (本文無し)                               | **#400: cascade で `is_deleted=True`**。`Tweet.soft_delete()` が `reposts.filter(type=REPOST).update(...)` を実行 |
| `QUOTE`              | **引用者の本文は残る**。元 tweet が表示されていた埋め込みカード部分が placeholder に差し替わる。                                     | 残る                                     | alive のまま (本文を持つ独立した発言なので cascade しない)                                                        |
...
- **本プロジェクトでの実装方針**:
  - **REPOST → cascade soft-delete (#400)**: `Tweet.soft_delete()` が `self.reposts.filter(type=REPOST).update(is_deleted=True, ...)` で同時に消す。これにより TL に「このツイートは削除されました」という意味のない tombstone が残らず、行ごと消える。元 tweet と派生 repost 双方が `is_deleted=True` になるので、TL serializer の `is_deleted=False` filter で自然に弾かれる。signals は飛ばないが repost_count 整合は元 tweet も削除済なので不要。
  - QUOTE → 派生 tweet は cascade しない (本文を持つ独立した発言)。`quote_of` を埋め込まず `quote_of_unavailable=True` のフラグを立て、フロント側で placeholder カードを描画
  - REPLY → 同様 cascade しない。`reply_to` 解決時に削除済みなら placeholder 表示
- 物理 DELETE (CASCADE) は引き続き避ける。`repost_of` は ER §2.5 通り `on_delete=SET_NULL` を維持。本仕様変更は **論理削除の cascade のみ**。
