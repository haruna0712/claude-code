...
## PoC ハーネスと評価軸（P2-01 / #204）

PoC スキャフォールドは [`apps/search/poc/`](../../apps/search/poc/) に置く。
合成 corpus 1,000 件 + クエリ 50 本 (`apps/search/poc/queries.py`) で両方の
バックエンドを `python -m apps.search.poc.run` から実機実行する。

実機ベンチで埋めるテーブル（PoC 判定根拠）:

| 評価軸 | pg_bigm + Lindera | Meilisearch | 採用基準 |
|---|---|---|---|
| **Recall@10** (相対) | TBD | TBD | 80%+ で合格、両者同等なら pg_bigm 優先 |
| **P95 latency** (10k corpus) | TBD ms | TBD ms | < 300ms 必須 |
| **Index size** | TBD MB | TBD MB | RDS 容量制約 (200GB) を超えないこと |
| **インデックス再構築時間** | TBD s | TBD s | 100s 未満が望ましい |
| **運用コスト** | $0 (RDS 内) | +$20/月 (EC2) | コスト差が開発価値を超えるか |
| **形態素境界精度** (JP) | TBD | TBD | Lindera vs Meilisearch 同等程度 |
| **コード断片ヒット率** | TBD | TBD | `def `, `import` 等の Recall@10 |

判定:
- **pg_bigm 採用**条件: 上記 7 軸すべてで Meilisearch 比 -10% 以内 → Status: `Accepted (pg_bigm + Lindera)`
- **Meilisearch 移行**条件: いずれかの軸で大幅劣後 (例 Recall < 60%、P95 > 500ms) → 別 ADR-000X を作成し本 ADR は `Superseded by 000X`

実機ベンチ（コンテナ起動 + 計測実行）は **本 PR (#204) のスコープ外**。
`bench_pg_bigm.py` と `bench_meilisearch.py` は scaffold で `NotImplementedError`
を返す状態。別 PR で psycopg2 + meilisearch SDK 経由の本実装を入れた上で
上記テーブルの値を埋める。
