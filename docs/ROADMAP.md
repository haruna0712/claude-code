...
| Phase 11  | メンターマッチング (Menta ライク、 募集 board / profile / review、 課金は無償ベータ pivot) | ✅ **コード完了** (11-A〜11-D + 11-F 全 Issue マージ済、 stg リリース済、 gan-evaluator 7.8/10、 chore P11-24/25/26 のみ残) — spec: [docs/specs/phase-11-mentor-board-spec.md](./specs/phase-11-mentor-board-spec.md)、 Issue: [docs/issues/phase-11.md](./issues/phase-11.md) |
...
- [x] `apps/dm` モデル実装 ← #226
- [x] Django Channels セットアップ（ASGI, Routing, Consumer）← #227 #228
- [x] 招待フロー API（作成・承諾・拒否）← #229
- [x] **S3 プリサインド URL 方式での直アップロード** (H-6) ← #231 #232
  - フロント → S3 直接アップロード
  - 完了後に URL だけサーバーへ送信
  - Django 経由での大容量アップロードを禁止（Channels イベントループ保護）
- [x] 既読管理（`last_read_at`）← #230
- [x] Block/Mute スタブブリッジ + dm_message/dm_invite 通知スタブ ← #240 (Phase 4B 完了で Block 部分は活性化済)

#### フロントエンド

- [x] DM 一覧 / 個別画面 ← #233 #234
- [x] WebSocket 接続、**reconnecting-websocket で再接続** ← #241
- [x] タイピング中表示（3 秒超で `role="status"` 一回告知、A11Y 準拠）← #242
- [x] 既読マーク ← #234
- [x] 画像・ファイルプレビュー（S3 直アップロード）← #235
- [x] グループ作成フロー、招待通知 → 承諾/拒否 UI ← #236 #237

#### インフラ・QA

- [x] Terraform: ALB target group `daphne` + ECS service `channels` ← #238
- [x] local.yml daphne サービス追加 ← #239
- [x] CloudWatch dashboard / alarm (/ws/\* 5xx, daphne CPU, channel layer queue) ← #243
- [x] a11y レビュー (a11y-architect) ← #244
- [x] セキュリティレビュー (WebSocket Origin/CSRF / room IDOR / プリサインド制約) ← #245
- [x] Playwright DM E2E (golden path) ← #246
- [ ] Phase 3 stg デプロイ + ADR-0005 起票 + SPEC/ER/ARCHITECTURE 更新 ← #247 (ハルナさん側 deploy 検証)

### 受け入れ基準

- [x] 1:1 + グループ（最大 20 名）DM リアルタイム動作
- [x] 既読・タイピング表示正常
- [x] S3 プリサインド URL 経由で画像・ファイル送信成功 (2026-05-07 API 統合検証: presign POST 200 → S3 multipart 204 → confirm 201 → WS send_message id=56 + attachment id=1 → test3 側で受信 + CloudFront 経由 GET 200 で 459927 bytes 完全一致)
- [x] Phase 末に stg へデプロイ、WebSocket が ALB 経由で動作 (`wss://ws.stg.codeplace.me/ws/dm/<room_id>/` で daphne TG 直結)
- [x] **本物の UI E2E** で添付フローが踏めること (2026-05-09 PR #466 / Issue #464、`dm-attachment-display.spec.ts`: setInputFiles で画像 → bubble の `<img>` → click → lightbox → ESC、PDF → file chip → ダウンロード anchor、stg 上で 2/2 pass。msg=58 att=3 で width/height=1x1 が DB 保存も確認)

### Phase 3 follow-up: 添付表示 (Teams 級 UX) — 2026-05-09 完了

ハルナさんの「画像を添付して送信したらメッセージ中で画像を見れるように、teams のチャットとかと同じように」要望への対応。8 Issue (#458-#465) を PR #466 で一括マージ。

- [x] backend: `MessageAttachmentSerializer` に `url` field、`ConfirmAttachmentInputSerializer` に optional `width/height`、`DM_ATTACHMENT_BASE_URL` env 追加 ← #458 #459 #460
- [x] frontend: `imageDimensions` util + `attachmentDisplay` util + `AttachmentImageGrid` (1〜5+ 枚 grid + 「+N」overlay) + `AttachmentFileChip` + `AttachmentLightbox` (Radix Dialog + focus trap + ←→ + reduced-motion) + `MessageBubble` 統合 ← #461 #462 #463
- [x] 本物の UI E2E (`dm-attachment-display.spec.ts`、setInputFiles 経由) ← #464
- [x] docs/specs/dm-attachment-display-{spec,scenarios,e2e-commands}.md ← #465
- [x] terraform `DM_ATTACHMENT_BASE_URL=https://${var.app_fqdn}` apply 済 (2026-05-09)
- [x] **compose preview をサムネイル表示に統一** (Slack/Discord/WhatsApp 標準 UX): chip だけだと体感が劣るため、画像 MIME のとき 72px サムネイル `<img>` + filename + size + × に変更。`MessageAttachment.url` (CloudFront) を直接 `<img src>` に使用 ← #469 / PR #471 (2026-05-09)
- [x] **`ConfirmAttachmentView` レスポンスに `url`/`width`/`height` を含める**: PR #458 で serializer に `url` 追加した際、confirm endpoint は手書き dict のままだったため compose preview の `a.url` が undefined で常に chip にフォールバックしていた。`MessageAttachmentSerializer(attachment).data` を返すよう修正、stg E2E ATT-IMG GREEN 確認 (2026-05-09 picture/dm-attach-shots/03-thumbnail-compose-preview.png) ← #473 / PR #474 (2026-05-09)
- [x] **既存 group room 内に「招待」ボタン追加** (Slack/Discord/Teams 標準導線): `/messages` トップに招待 link しかなく room 内から後追い招待できなかった。`RoomChat` ヘッダに `InviteMemberButton` (kind=group + creator のみ表示) と `InviteMemberDialog` (Radix Dialog + handle 入力) 追加。backend (`POST /api/v1/dm/rooms/<id>/invitations/`) は P3-04 で完成済、UI のみ追加。stg UI E2E `dm-room-invite.spec.ts` で test2 (creator) が test3 を招待 → test3 が承諾 → membership 反映 を 1/1 GREEN (2026-05-09 picture/dm-attach-shots/04-room-invite-button.png, 05-room-invite-dialog.png)。仕様書 `docs/specs/dm-room-invite-spec.md` 同梱 ← #476 / PR #477 (2026-05-09)
- [x] **Ctrl+V クリップボード画像 paste 添付**: MessageComposer の `onPaste` で clipboardData の image/\* を検出し `uploadAttachment` (presign → S3 → confirm) を自動実行。filename 空のためサーバ用に `pasted-<ts>.<ext>` を合成。テキスト paste には介入しない ← #470 / PR #482 (2026-05-10)
- [x] **room メンバー一覧 UI**: `RoomMembersButton` (group のみ visible) + `RoomMembersDialog` (Radix) で `room.memberships` を listing。creator バッジ + 参加日 ← #479 / PR #483 (2026-05-10)
- [x] **handle autocomplete (user search backend + UI)**: 新規 `GET /api/v1/users/?q=<prefix>&limit=N` (認証必須 / case-insensitive 前方一致 / self+inactive 除外 / limit 10/50)、`InviteMemberDialog` を combobox + listbox + 矢印 / Enter / click 補完に変更 (debounce 250ms、2 文字以上で fetch) ← #480 / PR #484 (2026-05-10)
- [x] **招待取消** (creator が pending invitation を取消): backend `DELETE /api/v1/dm/invitations/<id>/` 新規 + `InvitationListView` に `?as=invitee|inviter` filter 拡張、`InviteMemberDialog` に「送信中の招待」 section + 取消 button ← #481 / PR #485 (2026-05-10)
- [x] **通知 bridge 実装** (Phase 4A 着工): `apps/notifications/signals.py` に `emit_notification` を追加し、`apps/dm/integrations/notifications.py` 経由で `dm_invite` / `dm_message` が `Notification` に永続化されるよう修正。SPEC §7.5 / §8.1 のアプリ内通知配信を担保。stg curl 検証 + UI E2E (`dm-room-invite.spec.ts` に検証 step 追加) GREEN ← #487 / PR #488 (2026-05-10)
- [x] **通知から直接 承諾/拒否** (Slack/Discord/Teams/LINE 流の inline action): `/notifications` ページの `dm_invite` 通知 row に `DmInviteActions` (承諾/拒否 button) を inline 配置。click 一発で `accept`/`decline` API を叩き、成功時「参加しました」 status + 1.5s 後 row 自動消去 + read 化。受信箱 `/messages/invitations` への navigate 不要。stg 実機検証で test3 が inline 承諾だけで room 13 のメンバーになること確認 (memberships=["test2","test3"] 反映) ← #489 / PR #490 (2026-05-10)
- [x] **メンバー削除 (kick) + 退室 (leave) UI** (Slack/Discord/Teams/LINE 流): backend `DELETE /api/v1/dm/rooms/<id>/members/<user_id>/` 新規 (creator のみ可、target≠creator/non-member は 400)、`RoomMembersDialog` の各 member 行 (creator 視点) に削除 button + ダイアログ最下部に「このグループを退室」 button。pytest 8 + vitest 10 + stg 実機検証 (test2 が test3 を kick → memberships=["test2"]、test3 が leave → 404) ← #492 / PR #493 (2026-05-10)
- [x] **お気に入り (Google ブックマーク風 folder 階層)** (Phase 4A 一部先行): `apps/boxes` に `Folder` (self-FK で nest 可、MAX_DEPTH=10) と `Bookmark` を新設。`/api/v1/boxes/{folders,bookmarks,tweets/<id>/status}` 一式 + 認可 (本人以外 404 隠蔽、SPEC §9 完全非公開維持)。frontend は TweetCard の 🔖 → `AddToFolderDialog` (checkbox + 新規 folder 作成) + プロフィール「お気に入り」タブ (`?tab=favorites`、本人のみ表示)。pytest 19 / vitest 14 / E2E spec `client/e2e/favorites.spec.ts` (FAV-1/2)。仕様書 `docs/specs/favorites-spec.md` 同梱 ← #499 / PR #501 #502 #503 (2026-05-10)
...
- [x] `apps/boxes`（Folder + Bookmark、フォルダ階層) ← #499 / PR #501 #503 (Google ブックマーク風 UX、SPEC §9 拡張: 任意深さ folder ツリー、MAX_DEPTH=10)
- [x] 通知ベル UI（未読バッジ、A11Y 準拠 `aria-label`）← #412
- [x] 通知一覧画面 ← #412 / 設定画面（種別 ON/OFF）← 別 Issue
- [x] ボックス一覧・詳細、ブックマーク操作 ← #499 / PR #502 (TL 🔖 アイコン → AddToFolderDialog、プロフィール「お気に入り」タブ、本人のみ閲覧可)

### 受け入れ基準

- [x] 各イベントで通知発火 (#412 で 6 種別、Phase 3/5 で残り 4 種別追加予定) / 種別 ON/OFF は別 Issue
- [x] お気に入りボックスが非公開で動作 ← #499 (本人のみ閲覧、他人 folder は 404 隠蔽、SPEC §9 完全非公開)
...
- [x] `apps/articles`（Article, ArticleTag, ArticleImage, ArticleLike, ArticleComment） (P6-01, #511/#524)
  - **UNLISTED 状態は実装しない**（Q2 で限定公開を除外）
- [x] 記事 CRUD API (P6-03, #513/#526)
- [x] Markdown → HTML レンダリング + XSS ペイロードテストセット (P6-02, #512/#525)
- [x] 記事画像 S3 アップロード（プリサインド POST 方式 + confirm、`/api/v1/articles/images/{presign,confirm}/`） (P6-04, #527 / PR #591)
...
- [x] 記事作成・編集画面（Markdown エディタ、 live preview、 画像 D&D / paste / file picker） (P6-13 #536 / PR #545 stub + PR #601、 自動保存 / emoji picker / tag autocomplete は別 issue 持ち越し)
- [x] 記事一覧 `/articles` (P6-11, #534 / PR #545)
- [x] 記事詳細 `/articles/<slug>`（**未ログイン閲覧可**、OGP / JSON-LD + owner「編集/削除」 button + `/articles/me/drafts`） (P6-12 #535 / #593 / PR #594)
- [ ] いいね・コメント UI (P6-14)
- [ ] GitHub 連携設定画面（連携 ON/OFF、リポジトリ選択） (P6-15)
- [ ] 記事由来ツイートの 📄 マーク表示（Phase 2 UI 更新） (P6-16)
