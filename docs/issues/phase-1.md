...
- [ ] Custom `pre_save` signal で username 変更を reject (migration でのみ可)
- [ ] pytest: 有効 / 予約語 / 長さ外 / 記号含み / 変更試行 の 5 ケース
...
- [ ] アクティベーション済みユーザーに JWT 発行 → HttpOnly Cookie set (ADR-0003)
...
- **P1-13 ログイン/サインアップ UI**: djoser endpoints + Google OAuth ボタン (依存: P1-13a)
