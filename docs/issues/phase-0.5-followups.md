...
## F-15. [infra][backend] Django の pg_bigm / pg_trgm CREATE EXTENSION マイグレーション ✅ 完了 (P2-02 で対応 / GitHub #76 close)
...
- **Status**: P2-02 (`docs/issues/phase-2.md`) に内包。GitHub #76 は #177 (P2-02) を後継として close 済み。
...
- [x] `python manage.py migrate` で拡張が有効化
- [x] `SELECT extname FROM pg_extension;` に `pg_bigm` / `pg_trgm` が存在
