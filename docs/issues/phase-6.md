...
- **Estimate**: S
- **Depends on**: P6-01
- **Status**: ✅ **完了** (PR #591 で main マージ、 2026-05-11)。 仕様 [docs/specs/article-image-upload-spec.md](../specs/article-image-upload-spec.md) 参照。
- **Follow-up**: #588 (s3_presign 共通化) / #589 (orphan GC) / #590 (SecurityHeaders retrofit)
...
- [x] `POST /api/v1/articles/images/presign/` { filename, mime_type, size } → presigned **POST** URL (fields/policy/signature) + s3_key (auth、 DM 添付 `apps/dm/s3_presign.py` の流儀踏襲、 `apps/users/s3_presign.py` ではなく)
- [x] `POST /api/v1/articles/images/confirm/` { s3_key, filename, mime_type, size, width, height } → `head_object` で再検証 → ArticleImage 作成 + CloudFront URL を返す (auth)
- [x] 画像種類: `image/png|jpeg|webp|gif`、size <= 5MB
- [x] storage: `articles/<user_id>/<image_uuid>.<ext>` (Article 保存時の binding は P6-09 で再評価)
- [x] rate limit: `article_image_presign` 30/hour + `article_image_confirm` 30/hour (stg 300/hour)
- [x] CloudFront に `/articles/*` behavior + SecurityHeadersPolicy (nosniff) を attach (`terraform/modules/edge/main.tf`)

### 受け入れ基準

- [x] pytest: 画像 PUT 成功 → confirm → ArticleImage 作成 (T1-T8 + 単体テスト + boundary + Conditions assert + 二重 confirm = 36 ケース全 pass)
- [x] 5MB 超 / wrong content-type は 400
- [x] frontend は CloudFront URL を Markdown 内 `![](<url>)` に挿入 (frontend wiring は P6-13 follow-up PR C)
...
- **Estimate**: M
- **Depends on**: P6-03
- **Status**: ✅ **MVP 完了** (PR #545 で stub + OGP/JSON-LD、 PR #594 (#593) で owner 「編集/削除」 button 追加でループ閉)。 残「関連記事 もっと読む」 は別 issue。
...
- [x] `app/(template)/articles/[slug]/page.tsx` Server Component
- [x] `body_html` を表示 (DOMPurify は backend で済んでいるが念のため client-side でも sanitize)
- [x] `<head>`: `og:title` / `og:description` / `og:image` / Twitter Card
- [x] JSON-LD: schema.org `Article` (headline, author, datePublished, image)
- [x] **owner 専用「編集 / 削除」 button** (#593 で追加、 PR #594)
- [ ] サイドバー: 著者カード + 関連タグ + "もっと読む" (同じ著者の他記事) — 別 issue で対応
- [ ] 「いいね」「コメント」 セクション (P6-14 で実装、本 issue では placeholder)

### 受け入れ基準

- [x] view-source で og: タグと application/ld+json があることを assert (Playwright spec)
- [x] vitest: render、Markdown コードブロックがハイライト表示
...
- **Status**: ✅ **コア完了** (PR #545 stub + PR #601 で live preview + 画像 D&D 統合、 2026-05-11)。 自動保存 / emoji picker / tag autocomplete は別 issue 持ち越し。
- **仕様**: [docs/specs/article-editor-enhancements-spec.md](../specs/article-editor-enhancements-spec.md)
- **Follow-up**: #602 (画像 alt 編集 UI)
...
- [x] `app/(template)/articles/new/page.tsx` (auth 必須、未ログインは redirect login) (PR #545)
- [x] `app/(template)/articles/[slug]/edit/page.tsx` (本人のみ) (PR #545 + PR #594 で owner button + drafts)
- [x] エディタ: 左 textarea + 右プレビュー (split pane)、 `react-markdown` v9 + `remark-gfm` で rendered preview (PR #601)
- [x] 画像 D&D / paste / file picker: P6-04 (PR #591) の presigned URL → S3 直 POST → `![alt](<url>)` を caret 位置に挿入 (PR #601、 並列上限 3 の state machine + 30s timeout + validation)
- [x] フォーム: title / tags (5 個 max) / status (draft/published 切替) (PR #545)
- [ ] 自動保存: 30 秒ごと PATCH (draft のみ) — **別 issue 持ち越し** (race / debounce 設計が必要)
- [x] 「公開」 button: status=published に切替 → 確認 dialog (PR #545)
- [ ] tags は既存 Tag マスタから autocomplete — **別 issue 持ち越し**
- [ ] emoji picker — **別 issue 持ち越し**

### 受け入れ基準

- [x] vitest: form validation、 preview render、 画像 upload state machine、 D&D / paste handler (PR #601 で 36 ケース)
- [ ] stg 実機検証: 記事を新規作成 → 画像 D&D → 公開 → 一覧 + 詳細で確認 — **stg CD 反映後にハルナさん側で確認**
