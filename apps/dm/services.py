...
- :func:`mark_room_read` — ``last_read_at`` を message.created_at に揃える (P3-05、巻き戻し禁止)
- :func:`get_unread_count_for_room` — 指定 room の未読数 (P3-05)
- :func:`annotate_rooms_with_unread_count` — Subquery で room 一覧の未読数を inline annotate (P3-05)
...
# 1970-01-01 UTC: ``last_read_at`` が NULL のときに「全件未読」扱いするための fallback
# (annotate_rooms_with_unread_count の Coalesce で使う、再計算を避けるため module level)。
...
def mark_room_read(
    *,
    ...
    """``user`` の ``DMRoomMembership.last_read_at`` を ``message.created_at`` に揃える.

    SPEC §7.4: room 単位で ``last_read_at`` を保持し、それ以降のメッセージを未読扱い。

    バリデーション:

    - ``message`` が ``room`` 配下でなければ :class:`ValidationError` (IDOR + 越境防止)
    - ``user`` が ``room`` の member でなければ :class:`PermissionDenied`

    既存の ``last_read_at`` より古い時刻に巻き戻すことは許容しない (常に進む方向だけ)。
    ``Message.objects.create`` 直後の値を使うのが想定動作なので、巻き戻しは UI バグの兆候。
    """

    if message.room_id != room.pk:
        raise ValidationError("message が指定 room 配下ではありません")

    membership = DMRoomMembership.objects.filter(room=room, user=user).first()
    if membership is None:
        raise PermissionDenied("このルームのメンバーではありません")

    new_last_read_at = message.created_at

    # review HIGH 反映: read-check-write race を排除するため DB 側 conditional
    # UPDATE で `last_read_at IS NULL OR last_read_at < new` のみ書き込む。
    # 同等 (==) の既読位置は no-op (idempotent)。
    DMRoomMembership.objects.filter(pk=membership.pk).filter(
        Q(last_read_at__isnull=True) | Q(last_read_at__lt=new_last_read_at)
    ).update(
        last_read_at=new_last_read_at,
...
def get_unread_count_for_room(*, room: DMRoom, user: AbstractBaseUser) -> int:
    """``room`` における ``user`` の未読メッセージ数を返す.

    定義 (SPEC §7.4):

    - ``Message.created_at > membership.last_read_at`` のメッセージのみ
    - 自分が送信した message は未読にカウントしない
    - 削除済 (``deleted_at`` not null) は未読にカウントしない
    - membership が無い場合は 0 (member でない room は未読を語る意味が無い)
    """
    ...
    qs = Message.objects.filter(room=room, deleted_at__isnull=True).exclude(sender=user)
    if membership.last_read_at is not None:
        qs = qs.filter(created_at__gt=membership.last_read_at)
    return qs.count()


def annotate_rooms_with_unread_count(
    rooms_qs: QuerySet[DMRoom], user: AbstractBaseUser
) -> QuerySet[DMRoom]:
    """Room queryset に ``unread_count`` annotation を付ける (Subquery で N+1 回避).

    room 一覧 API が呼ぶ前提。``rooms_qs`` の各 room につき、当該 user が読むべき
    未読メッセージ数 (``last_read_at`` 以降 + sender != user + deleted_at is null) を
    1 クエリで計算する。

    実装方針 (database-reviewer N+1 防御):

    1. 各 room の ``last_read_at`` を ``user_last_read`` 名で flat 構造に annotate
       (Subquery を入れ子にすると ``OuterRef`` の解決層が壊れるため)
    2. ``Message`` への correlated subquery で ``COUNT`` を取り、``OuterRef`` で
       outer の ``user_last_read`` を参照
    3. ``last_read_at`` が NULL のときは ``_EPOCH`` (1970-01-01) にフォールバックして
       「全件未読」扱い (``Coalesce``)
    """

    # 1. outer rooms_qs に当該 user の last_read_at を annotate (NULL fallback も
    # Coalesce で済ませる)。これで rooms_qs の各 room は ``user_last_read`` 列を
    # 持つようになり、後段の subquery から ``OuterRef("user_last_read")`` で参照可能。
    last_read_sq = DMRoomMembership.objects.filter(room=OuterRef("pk"), user=user).values(
        "last_read_at"
    )[:1]

    rooms_with_last_read = rooms_qs.annotate(
        user_last_read=Coalesce(
            Subquery(last_read_sq, output_field=DateTimeField()),
            _EPOCH,
            output_field=DateTimeField(),
        )
    )

    # 2. 各 room の未読 message を数える correlated subquery。outer の
    # ``user_last_read`` を OuterRef で参照する。
    unread_sq = (
        Message.objects.filter(room=OuterRef("pk"), deleted_at__isnull=True)
        .exclude(sender=user)
        .filter(created_at__gt=OuterRef("user_last_read"))
        .order_by()
        .values("room")
        .annotate(c=Count("pk"))
        .values("c")
